...
let DataService = class DataService {
    constructor(handler, httpClient) {
    ...
    fetchData() {
        ...
        return this.httpClient.get('https://covid19.mohp.gov.np/covid/api/confirmedcases/?format=json').pipe(data => {
            return data;
        });
    }
    setdata(data) {
...
let LandingPageComponent = class LandingPageComponent {
    constructor(modal, router, viewportScroller, dataService) {
        this.modal = modal;
        this.router = router;
    ...
    counter(id, start, end, duration, ste) {
        let obj = document.getElementById(id), current = start, range = end - start, increment = end > start ? ste : -1, step = Math.abs(Math.floor(duration / range)), timer = setInterval(() => {
            current += increment;
...
let VisualizenepalComponent = class VisualizenepalComponent {
    constructor(apicall, handler, httpClient, dataService) {
        this.apicall = apicall;
        this.handler = handler;
        this.httpClient = httpClient;
        this.dataService = dataService;
